...
- Every write/delete triggers the broadcast trigger, which publishes a lightweight text payload (event flag + version + unit-separator + escaped key) to that channel (defaults to `cache_events`).
